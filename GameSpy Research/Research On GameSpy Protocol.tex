...
\label{Login diagram}
\end{figure}
We show the common key/value pairs in \ref{Common login keys}.
\begin{table}[H]
	\centering
	\begin{tabular}{A}
		...
		\hline 
		final & message end delimiter&  \\ 		
		\hline 
		id & the operation number that client generate &\\
		\hline 
	\end{tabular} 
	\caption{Common login keys}
	\label{Common login keys}	
\end{table}	
...
\begin{tcolorbox}
	...
	\begin{split}
	\backslash lc \backslash 1 \backslash challenge \backslash \langle \rangle 	\backslash nur \backslash userid \backslash \langle \rangle \backslash profileid \backslash \langle \rangle \backslash final \backslash
	\end{split}
	\end{equation}
\end{tcolorbox}
\begin{table}[H]
...
\begin{table}[H]
	...
	\begin{tabular}{A}
		...
		challenge  & The user challenge used to verify the authenticity of the client     &10 byte alphanumeric string                                                                                                                                     \\ \hline
		...
		lt& The login ticket used for login into SAKE& \\ \hline
		id& The operation number&\\ \hline
	\end{tabular} 
