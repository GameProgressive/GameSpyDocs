...
\begin{figure}
	...
	\begin{tikzpicture}
		\begin{package}{Architecture}
			...
			\begin{class}[text width = 3cm]{UserInfo}{5,-10}
			\attribute{String : nickname}
			\end{class}
			
			\begin{class}[text width = 4cm]{ChatSession}{5,-5}
			\attribute{UserInfo : User}
			\operation{+ CreateChatChannel()}
			\operation{+ UserLogin()}
			\operation{+ JoinChatChannel()}
			\end{class}
	

			\begin{class}[text width = 4cm]{ChatChannel}{-5,-5}
			\operation{+ AddChatSession()}
			\operation{+ DeleteChatSession()}
			\operation{+ BanChatSession()}
			\end{class}
			
			\composition{ChatServer}{}{}{ChatSession}
			\composition{ChatServer}{}{}{ChatChannel}
			\aggregation{ChatSession}{}{}{ChatChannel}
			\composition{ChatSession}{}{}{UserInfo}
		\end{package}
	\end{tikzpicture}
\end{figure}
...
\begin{figure}
	\begin{tikzpicture}
		...
		\attribute{String: Messages}
		\end{class}
		\begin{class}[text width=4cm]{IRCBasicCommand}{0,-3}
