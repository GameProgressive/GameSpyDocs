...
\begin{figure}[H]
	...
	\begin{sequencediagram}
		...
		\newinst[4]{NN}{:NatNeg}{}
		\begin{call}
			{Client}{External reachable test1()}{NN}{ERT response1()}
		\end{call}
		\begin{call}
			{Client}{External reachable test2()}{NN}{ERT response2()}
		\end{call}
			\begin{call}
		{Client}{External reachable test3()}{NN}{ERT response3()}
	\end{call}
		\begin{call}
	{Client}{Address mapping test1()}{NN}{Address mapping response1()}
\end{call}
		\begin{call}
	{Client}{Address mapping test2()}{NN}{Address mapping response2()}
\end{call}
		\begin{call}
	{Client}{Address mapping test3()}{NN}{Address mapping response3()}
\end{call}
	\end{sequencediagram}
	\caption{NatNeg ERT and address mapping test sequence diagram}
\end{figure}
...
\section{Query Report Packet}

\section{Heart Beat Packet}

\section{Challenge Packet}

\chapter{Heart Beat Request}
\begin{figure}[H]
	\centering
	\begin{sequencediagram}
		\newinst{Client}{:Client}{}
		\newinst[3]{QR}{:QR}{}
		\begin{call}
			{Client}{Available check()}{QR}{}
		\end{call}
		\begin{call}
			{Client}{Hear beat report()}{QR}{Challenge()}
		\end{call}
		\begin{messcall}
			{Client}{Challenge response()}{QR}{}
		\end{messcall}
		\begin{call}
			{QR}{Echo packet()}{Client}{Echo packet()}
		\end{call}
	\end{sequencediagram}
...
\chapter{Server List Retrieve}

The first two bytes is the total length of the data
byte 3: command id (0 = request list)
byte 4: protocol version
byte 5: encoding version (3 = enctypeX)
4bit little endian int: game version
null terminated string: for game name
null terminated string: from game name
8 bytes: clientvalidate
null terminated string: server filter
null terminated string: field list (?)
4 byte int: options
if (options \& ALTERNATE\_SOURCE\_IP)
{
	4 byte int: source ip (?) 		
}
if (options \& LIMIT\_RESULT\_COUNT)
{
	4 byte int: max servers		
}
